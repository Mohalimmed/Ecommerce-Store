     ...
     <div class="shopping-item">
         ...
         <ul class="shopping-list">
             @foreach ($items as $item)
                 <li id="{{ $item->id }}">
                     <a href="javascript:void(0)" data-id="{{ $item->id }}" class="remove remove-item"><i
                             class="lni lni-close"></i></a>
                     <div class="cart-img-head">
                         <a class="cart-img" href="{{ route('prodcuts.show', $item->product->slug) }}"><img
                                 src="{{ $item->product->image_url }}" alt="#"></a>
                     </div>

                     <div class="content">
                         <h4><a href="{{ route('prodcuts.show', $item->product->slug) }}">
